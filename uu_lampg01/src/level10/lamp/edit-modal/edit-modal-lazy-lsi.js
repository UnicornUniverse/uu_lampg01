export const Lsi = {
  properties: {
    cs: "Lampa",
    en: "Lamp",
  },
  baseUri: {
    cs: "URL aplikace Lamp",
    en: "Lamp Applicaton URL",
  },
  code: {
    cs: "Identifikační kód",
    en: "Identification code",
  },
  codePatternMessage: {
    cs: "Kód může obsahovat pouze alfanumerické znaky a podtržítko. Nesmí být delší než 32 znaků.",
    en: "Code can contain only alphanumeric characters and underscore. Must be no longer then 32 characters.",
  },
  state: {
    cs: "Stav",
    en: "State",
  },
  on: {
    en: "on",
    cs: "zapnuto",
  },
  off: {
    en: "off",
    cs: "vypnuto",
  },
  bulbStyle: {
    cs: "Styl žárovky",
    en: "Bulb Style",
  },
  bulbSize: {
    cs: "Velikost žárovky",
    en: "Bulb Size",
  },
  header: {
    cs: "Záhlaví",
    en: "Header",
  },
  visual: {
    cs: "Vzhled",
    en: "Visual",
  },
  cardView: {
    cs: "Zobrazit jako kartu",
    en: "Card View",
  },
  colorSchema: {
    cs: "Barevné schéma",
    en: "Color Schema",
  },
  borderRadius: {
    cs: "Zaoblení rohů",
    en: "Border Radius",
  },
  elevation: {
    cs: "Elevation",
    en: "Elevation",
  },
  bgStyle: {
    cs: "Styl pozadí",
    en: "Background Style",
  },
  info: {
    cs: "<uu5string/>Více informací viz <UU5.Bricks.Link href='%s' target='_blank' content='dokumentace'/>.",
    en: "<uu5string/>For more information see <UU5.Bricks.Link href='%s' target='_blank' content='documentation'/>.",
  },
};

export default Lsi;
